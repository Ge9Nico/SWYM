...
export default function LoginForm() {
    ...
    const handlePasswordReset = async () => {
        const emailForReset = prompt("Please enter your email address to reset your password:");
        if (emailForReset) {
            try {
    ...
    return (
        <form onSubmit={handleLogin} className="space-y-4">
            <button type="button" onClick={handleGoogleLogin} disabled={loading} className="w-full flex items-center justify-center py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50">
            ...
            <div>
                ...
                <input type="email" id="login-email" value={email} onChange={e => setEmail(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" />
            </div>
            <div>
                <label htmlFor="login-password" className="block text-sm font-medium text-gray-700">Password</label>
                <input type="password" id="login-password" value={password} onChange={e => setPassword(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" />
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white py-2 px-4 rounded-lg font-semibold hover:bg-blue-700 transition flex items-center justify-center disabled:opacity-50">
                {loading ? <div className="loader h-5 w-5 border-2 border-t-2 rounded-full"></div> : 'Log In'}
            </button>
            <a href="#" onClick={handlePasswordReset} className="text-sm text-blue-600 hover:underline text-center block">Forgot Password?</a>
        </form>
    );
}